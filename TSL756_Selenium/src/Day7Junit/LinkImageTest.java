...
import java.net.Proxy; // FOR PROXY SETTING
import java.net.URL;
...
public class LinkImageTest 
{
	...
	public void test() 
	{
		driver.get("https://www.urbanclap.com/");
		...
		for(WebElement E: alllinks)
		{
			...
			try {
				Proxy proxy = new Proxy(Proxy.Type.HTTP, //no need to write this line when using system at home
						new InetSocketAddress("192.168.100.1", 8081));
				URL U=new URL(url);
				URLConnection C=U.openConnection(proxy); //OPEN THE CONNECTION USING PROXY
				HttpURLConnection HC=(HttpURLConnection)C;// URL CONNECTION
				HC.connect();
